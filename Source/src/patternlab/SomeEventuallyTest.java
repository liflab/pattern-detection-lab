...
public class SomeEventuallyTest
{
	@Test
	public void test1()
	{
		IndexEventTracker tracker = new IndexEventTracker();
		SomeEventually ae = new SomeEventually(3);
		...
		parents = tracker.getProvenanceTree(ae, 0, 1).getParents();
		assertEquals(1, parents.size());
		isDataPoint((InputValue) parents.get(0).getNodeFunction(), 0, 1);
		p0.push(FALSE); p1.push(INCONCLUSIVE); p2.push(INCONCLUSIVE);
		last = sink.getLast();
		assertEquals(TRUE, last[0]);
		parents = tracker.getProvenanceTree(ae, 0, 2).getParents();
		assertEquals(1, parents.size());
		isDataPoint((InputValue) parents.get(0).getNodeFunction(), 0, 1);
		p0.push(TRUE); p1.push(TRUE); p2.push(INCONCLUSIVE);
		last = sink.getLast();
		assertEquals(TRUE, last[0]);
		parents = tracker.getProvenanceTree(ae, 0, 3).getParents();
		assertEquals(1, parents.size());
		isDataPoint((InputValue) parents.get(0).getNodeFunction(), 1, 3);
		p0.push(TRUE); p1.push(INCONCLUSIVE); p2.push(TRUE);
		last = sink.getLast();
		assertEquals(TRUE, last[0]);
		parents = tracker.getProvenanceTree(ae, 0, 4).getParents();
		assertEquals(1, parents.size());
		isDataPoint((InputValue) parents.get(0).getNodeFunction(), 2, 4);
	}
