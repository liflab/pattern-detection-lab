 ...
 */
public class FindOccurrences extends SynchronousProcessor
{
	/**
	 * The processor acting as a monitor to detect pattern instances.
	 */
	/*@ non_null @*/ protected Monitor m_pattern;
	 ...
	 * instances
	 */
	public FindOccurrences(/*@ non_null @*/ Monitor pattern)
	{
		super(1, 1);
		m_pattern = pattern;
	...
	{
		Monitor m_dup = m_pattern.duplicate(false);
		...
		if (!matches.isEmpty())
		{
			...
			if (m_eventTracker != null)
			{
				...
				{
					List<Integer> subseq = m.m_monitor.getSequence();
					for (int pos : subseq)
					{
	...
	@Override
	public FindOccurrences duplicate(boolean with_state)
	{
		FindOccurrences fp = new FindOccurrences(m_pattern.duplicate(with_state));
		if (with_state)
		{
	...
	public class PatternInstance
	{
		 ...
		 * The processor acting as a monitor to detect pattern instances.
		 */
		/*@ non_null @*/ protected Monitor m_monitor;
		...
		public PatternInstance(/*@ non_null @*/ Monitor monitor, int start_index)
		{
			super();
			m_monitor = monitor;
